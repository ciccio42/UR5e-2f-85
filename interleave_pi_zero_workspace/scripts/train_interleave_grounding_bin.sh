...
run_training() {
    ...
    local run_name="interleave-ur5e-bin-grounding-${mode}"
...
run_container() {
    ...
    docker run --rm \
        ...
        \
        "$IMAGE" \
        bash "$CONTAINER_WORKSPACE/scripts/train_interleave_grounding_bin.sh" \
            --inside "$mode"
}
...
start_tmux() {
    ...
    SESSION_NAME="${SESSION_NAME:-interleave-ur5e-bin-grounding${tag_suffix}-${mode}}"

    CONTAINER_NAME="${CONTAINER_NAME:-interleave-ur5e-bin-grounding${tag_suffix}-${mode}}"
